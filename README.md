...
LLM API Factory 是一个面向个人和小团队的 LLM API 网关。它把 OpenAI、Anthropic、Gemini、隔离的 Codex OAuth 渠道和自定义上游统一到一套控制面里，负责 API key 管理、模型路由、故障切换、用量观测，以及最重要的远程 Agent 代理执行。
...
- **Codex OAuth 隔离渠道**：支持在创建端点或 Key 管理界面粘贴/上传 Auth JSON，并按 Key 展示 5 小时与 1 周用量窗口。
- **Agent 统一网络出口**：渠道绑定远程 Agent 后，模型探测、Key 测试、Token 刷新和真实推理请求都由该 Agent 发出；未绑定时由主服务直连。
...
flowchart LR
    ...
    Direct --> UpstreamA[OpenAI / Anthropic / Gemini / Codex / Custom]
...
当前按新项目方式运维：空数据库会根据当前 ORM 模型创建完整 schema，不承诺把任意旧版本数据库自动升级到最新版。已有部署升级前请先备份，并自行执行与当前模型匹配的 SQL，或使用新的空数据库。
...
5. 下游继续调用 Factory，该渠道的探测、测试、Token 刷新和真实请求均由远程 Agent 发出。
...
curl -fsSL https://raw.githubusercontent.com/Kuaizr/llm_api_factory/master/scripts/agent_install.sh | bash -s -- \
  ...
  --allowed-targets "api.openai.com,api.anthropic.com,generativelanguage.googleapis.com,chatgpt.com,auth.openai.com"
```
...
- [API 入口](docs/api.md)
- [Codex OAuth Provider](docs/codex-provider.md)
...
- Codex OAuth 是隔离的实验 provider，不会混入标准 OpenAI API Key 渠道；不提供任意订阅账号 OAuth 到 API 的通用转换。
