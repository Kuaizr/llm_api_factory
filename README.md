...
- OpenAI 风格入口：models、chat completions、completions、embeddings、responses 和 `/openai/v1/*` 通用透传
- Anthropic 风格入口：models、messages 和 `/anthropic/v1/*` 通用透传
- Gemini 风格入口：models、generateContent、streamGenerateContent、interactions 和 `/gemini/v1/*`、`/gemini/v1beta/*` 通用透传
- 标准链路流式与非流式透传
- 按模型名和规则组选择候选 endpoint / key
- sequential、weighted round robin 等策略
- 熔断不可用 key，并在探测或成功请求后恢复
- Agent 节点通过 WebSocket 回连，支持远程代理请求
- dump_index 记录 token、cached tokens、cache hit、stream complete 和 Gemini previous interaction
- 管理控制台和 CLI 两套控制面
...
当前项目按个人或小团队规模设计，默认取舍如下：

- SQLite 适合低并发和单机部署；请求日志写入量上来后应迁移 PostgreSQL。
- 路由顺序和 weighted round robin 的部分状态仍以单进程语义为主；多 worker 扩容前需要把这类运行态统一迁到 Redis。
- OpenAI / Anthropic / Gemini 入口不做跨 provider 协议翻译；统一入口的价值是 key 管理、路由、fallback 和 agent 执行位置控制。
- 同一逻辑 model alias 可以在不同 endpoint 映射到不同 real model，质量一致性由配置者管理。
- 健康探针会真实请求上游，可能消耗少量额度；需要低成本运行时可调大间隔或禁用单个 endpoint 的自动探针。
- 非流式请求在连接错误或特定状态码下会重试/fallback；如果上游超时但实际已处理，可能产生重复扣费，需按上游特性设置超时和候选顺序。
- 流式请求中途断开时，下游连接会随之结束；系统会尽量记录已收到的 usage/chunk，但不能把已开始的 SSE 响应改写成新的 JSON 错误。
- Agent 断线时，经路由筛选后会尽量 fallback 到其他可用候选；没有候选时返回错误。
- Telegram 告警有短重试，但不是强可靠告警通道；生产关键告警建议另接外部 webhook/监控系统。
...
管理台的 API key 页面可以对单个上游 endpoint/key 做测试，不经过 factory 规则组路由；如果该 endpoint 配置为 `via_agent`，测试请求也会通过绑定的 agent 发起。测试 prompt 固定为：
...
这个功能用于确认上游 endpoint/key 本身以及它的执行位置是否可用；正式下游调用仍通过 factory access key 和规则组路由。
...
`/openai/v1/models` 会尽量请求上游原生 models 接口，并按 factory access key 可访问的所有规则组取并集过滤。其他 `/openai/v1/{path}` 请求走最小侵入透传：body 默认不删字段、不做模板重写，只在需要路由时替换 `model`，并注入上游鉴权和 trace header。
...
`/anthropic/v1/models` 同样优先透传上游模型列表，并按 factory access key 可访问模型过滤。`/anthropic/v1/messages` 和其他 `/anthropic/v1/{path}` 入口使用 Anthropic 标准鉴权头转发，body 保持最小改写。
...
转发给上游时会剥离 factory key，并注入真实 Gemini API key。`/gemini/v1/models` 和 `/gemini/v1beta/models` 返回尽量贴近原生 Gemini models 的结构，同时按 factory access key 可访问模型过滤。`generateContent` 和 `streamGenerateContent` 会按路径里的 `{model}` 做路由并替换为真实上游 model。`interactions` 入口用于 Gemini Interactions API，body 里的 `model` 或 `agent` 用于路由，`previous_interaction_id` / `previousInteractionId` 会写入 dump_index 便于追踪缓存链路。
